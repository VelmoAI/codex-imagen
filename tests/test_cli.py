...
``codex_imagen.cli.forge`` to return a deterministic :class:`ForgeResult`
...
from codex_imagen.core import ForgeHealth, ForgeImage, ForgeResult
...
def _ok_health() -> ForgeHealth:
    """A healthy ForgeHealth — used for the default fake-result fixture."""
    return ForgeHealth(
        ok=True,
...
def _make_image(index: int, path: Path) -> ForgeImage:
    """Build a deterministic ForgeImage for use in fake results."""
    return ForgeImage(
        index=index,
        path=path,
        bytes=1024 * (index + 1),
...
def _ok_result(tmp_path: Path, n: int = 1) -> ForgeResult:
    """Construct an OK ForgeResult with ``n`` images for a fake forge() call."""
...
def _fail_result_generation(tmp_path: Path) -> ForgeResult:
...
class FakeForge:
    """Records its kwargs and returns a pre-baked ForgeResult.

    The CLI calls :func:`codex_imagen.cli.forge`, which we monkeypatch
    ...
    def __init__(self, result_factory: Any) -> None:
        # ``result_factory`` is a callable that builds the ForgeResult on
        # demand so test fixtures can use tmp_path lazily.
        self._result_factory = result_factory
        self.calls: list[dict[str, Any]] = []

    def __call__(self, **kwargs: Any) -> ForgeResult:
...
@pytest.fixture
def patched_forge_ok(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> FakeForge:
    """Replace cli.forge with a fake that always returns one OK image."""
    fake = FakeForge(lambda: _ok_result(tmp_path, n=1))
    monkeypatch.setattr(cli_mod, "forge", fake)
    return fake
...
def test_cli_help_works(runner: CliRunner) -> None:
    ...
    result = runner.invoke(cli_mod._forge_command, ["--help"])
...
def test_cli_version_prints_version(runner: CliRunner) -> None:
    ...
    result = runner.invoke(cli_mod._forge_command, ["--version"])
    assert result.exit_code == 0
...
def test_cli_no_args_exits_3(runner: CliRunner) -> None:
    ...
    result = runner.invoke(cli_mod._forge_command, [])
...
def test_cli_simple_prompt_pretty_mode(
    runner: CliRunner, patched_forge_ok: FakeForge
) -> None:
    """`forge "cat" --pretty` exits 0 and shows the pretty summary."""
    result = runner.invoke(cli_mod._forge_command, ["cat", "--pretty"])
    assert result.exit_code == 0
    assert "[OK]" in result.output
    assert len(patched_forge_ok.calls) == 1
    assert patched_forge_ok.calls[0]["prompt"] == "cat"


def test_cli_simple_prompt_json_mode(
    runner: CliRunner, patched_forge_ok: FakeForge
) -> None:
    """`forge "cat" --json` emits a single JSON object on stdout."""
    result = runner.invoke(cli_mod._forge_command, ["cat", "--json"])
    assert result.exit_code == 0
...
def test_cli_json_pretty_both_set_pretty_wins(
    ...
    result = runner.invoke(
        cli_mod._forge_command, ["cat", "--json", "--pretty"]
    )
    assert result.exit_code == 0
    assert "[OK]" in result.output
...
def test_cli_file_input_three_prompts(
    ...
    """`-f FILE` with three lines passes a list of three prompts to forge()."""
    f = tmp_path / "prompts.txt"
    f.write_text("alpha\nbeta\ngamma\n", encoding="utf-8")
    result = runner.invoke(
        cli_mod._forge_command, ["--json", "-f", str(f)]
    )
    assert result.exit_code == 0
    assert patched_forge_ok.calls[0]["prompt"] == ["alpha", "beta", "gamma"]
...
def test_cli_file_input_skips_blanks_and_comments(
    ...
    assert result.exit_code == 0
    assert patched_forge_ok.calls[0]["prompt"] == ["alpha", "beta"]
...
def test_cli_prompt_json_dict(
    ...
    result = runner.invoke(
        cli_mod._forge_command, ["--json", "--prompt-json", str(f)]
    )
    assert result.exit_code == 0
    assert patched_forge_ok.calls[0]["prompt"] == {"Primary request": "x"}
...
def test_cli_prompt_json_list(
    ...
    assert result.exit_code == 0
    assert patched_forge_ok.calls[0]["prompt"] == ["a", "b"]
...
def test_cli_multiple_references(
    ...
    """``-r`` may appear multiple times; forge() receives a tuple of paths."""
    a = tmp_path / "a.png"
    b = tmp_path / "b.png"
    a.write_bytes(b"x")
    b.write_bytes(b"y")
    result = runner.invoke(
        cli_mod._forge_command,
        ["cat", "--json", "-r", str(a), "-r", str(b)],
    )
    assert result.exit_code == 0
    refs = patched_forge_ok.calls[0]["references"]
    assert len(refs) == 2
...
def test_cli_var_parsing(
    ...
    """``--var key=value`` is parsed into the ``vars`` dict on ForgeOptions."""
    result = runner.invoke(
        cli_mod._forge_command,
        ["cat", "--json", "--var", "char=alice", "--var", "color=red"],
    )
    assert result.exit_code == 0
    assert patched_forge_ok.calls[0]["vars"] == {
        "char": "alice",
        "color": "red",
    }
...
def test_cli_var_invalid_exits_3(
    ...
    """A ``--var`` without ``=`` is a user error → exit 3, no forge() call."""
    result = runner.invoke(
        cli_mod._forge_command,
        ["cat", "--json", "--var", "lonely"],
    )
...
def test_cli_advanced_parsing(
    ...
    assert result.exit_code == 0
    assert patched_forge_ok.calls[0]["advanced"] == {
...
def test_cli_chroma_no_despill_flag(
    ...
    assert result.exit_code == 0
    assert patched_forge_ok.calls[0]["chroma_despill"] is False
...
def test_cli_chroma_despill_default_true(
    ...
    result = runner.invoke(cli_mod._forge_command, ["cat", "--json"])
    assert result.exit_code == 0
    assert patched_forge_ok.calls[0]["chroma_despill"] is True
...
def test_cli_health_ok_exits_0(
    ...
    """``--health`` with a healthy bridge exits 0 and never calls forge()."""
    monkeypatch.setattr(
        cli_mod._bridge,
        "health_check",
        lambda: {"ok": True, "hint": None},
    )
    result = runner.invoke(cli_mod._forge_command, ["--health", "--json"])
    assert result.exit_code == 0
    payload = json.loads(result.output)
    assert payload["ok"] is True
    assert patched_forge_ok.calls == []
...
def test_cli_forge_returns_ok_false_health_fail_exits_1(
    runner: CliRunner,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """A forge() result whose health is also failing maps to exit 1."""
    fake_health = ForgeHealth(
        ok=False,
        codex_image_gen_available=True,
        pillow_available=True,
    ...
    monkeypatch.setattr(cli_mod, "forge", lambda **kw: fake_result)
    result = runner.invoke(cli_mod._forge_command, ["cat", "--json"])
    assert result.exit_code == cli_mod.EXIT_HEALTH_FAILURE


def test_cli_forge_returns_ok_false_exits_2(
    runner: CliRunner,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """forge() ok=False with healthy bridge → exit 2 (generation failure)."""
    monkeypatch.setattr(
        cli_mod, "forge", lambda **kw: _fail_result_generation(tmp_path)
    )
    result = runner.invoke(cli_mod._forge_command, ["cat", "--json"])
...
def test_cli_quiet_human_mode_single_line(
    ...
    result = runner.invoke(
        cli_mod._forge_command, ["cat", "--pretty", "--quiet"]
    )
    assert result.exit_code == 0
...
def test_cli_quiet_json_still_emits_full_payload(
    ...
    result = runner.invoke(
        cli_mod._forge_command, ["cat", "--json", "--quiet"]
    )
    assert result.exit_code == 0
    payload = json.loads(result.output)
    assert payload["ok"] is True
...
def test_cli_invalid_mode_exits_2_click(
    ...
    result = runner.invoke(cli_mod._forge_command, ["cat", "--mode", "bogus"])
    assert result.exit_code == 2  # Click default for usage errors
    assert patched_forge_ok.calls == []


# ---------------------------------------------------------------------------
# 8) Error rescue paths (non-UTF-8 input, forge() raising ValueError)
# ---------------------------------------------------------------------------
...
def test_cli_file_with_invalid_encoding_exits_3(
    ...
    result = runner.invoke(cli_mod._forge_command, ["-f", str(bad)])
    assert result.exit_code == cli_mod.EXIT_INVALID_ARGS
...
def test_cli_forge_raises_value_error_exits_3(
    runner: CliRunner,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """When forge() raises ValueError (bad ForgeOptions), CLI exits 3.
    ...
    def boom(**kwargs: Any) -> ForgeResult:
        raise ValueError("bad size suggestion")

    monkeypatch.setattr(cli_mod, "forge", boom)
    result = runner.invoke(cli_mod._forge_command, ["cat", "--json"])
    assert result.exit_code == cli_mod.EXIT_INVALID_ARGS
